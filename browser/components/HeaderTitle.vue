<template>
  <div class="header-title">
    <img v-if="logoSrc" :src="logoSrc" :alt="logoAlt" :title="logoAlt" class="logo">
...
export default {
  ...
  computed: {
    ...
    // stac-js getAbsoluteUrl() returns null for path-only hrefs (no "://").
    // Path-absolute catalogImage values (e.g. /data/stac/browser/planet-logo.svg) must be used as-is.
    logoSrc() {
      if (this.catalogImage) {
        const absolute = this.logo?.getAbsoluteUrl?.();
        return absolute || this.catalogImage;
      }
      return this.logo?.getAbsoluteUrl?.() || null;
    },
    logoAlt() {
      return this.logo?.title || this.catalogTitle || '';
    },
    fallbackTitle() {
